...
download_crane() {
  ...
  case ${DOWNLOADER} in
  *curl)
    curl -L -o "${ARCHIVE_NAME}" https://github.com/google/go-containerregistry/releases/download/v0.20.6/"${ARCHIVE_NAME}"
    ;;
  *wget)
    wget https://github.com/google/go-containerregistry/releases/download/v0.20.6/"${ARCHIVE_NAME}"
    ;;
  *)
...
while read -r image; do
  ...
  ./crane pull --platform "${TARGETPLATFORM}" "$image" ${IMAGES_DIR}/"${name}"-"${version}".tar
done < $IMAGE_FILE
